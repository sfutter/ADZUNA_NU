...
dropCols = which(names(adzuna) %in% c('FullDescription', 'LocationRaw', 'SalaryRaw'))
s.adzuna = adzuna[,-dropCols]

# new data set has 9 columns instead of 12
dim(s.adzuna)  # 244768 x 9
...
histogram(~ SalaryNormalized | factor(ContractType), data = s.adzuna)
histogram(~ SalaryNormalized | factor(ContractTime), data = s.adzuna)
```
...
```{r}
library('dplyr')
mean.salary = s.adzuna %>%         # treat %>% as a 'then' statement... 
  group_by(Category) %>%
  summarise(
